...
class Page extends EventEmitter {
  ...
  async _screenshotTask(format, options) {
    ...
    if (options.omitBackground)
      await this._setBackgroundTransparent(true);
    const result = await this._client.send('Page.captureScreenshot', { format, quality: options.quality, clip });
    if (options.omitBackground)
      await this._setBackgroundTransparent(false);
  ...
  async _setBackgroundTransparent(transparent) {
    if (transparent)
      return this._client.send('Emulation.setDefaultBackgroundColorOverride', { color: { r: 0, g: 0, b: 0, a: 0 } });
    else
